...
void Driver_Stm_Init(void) {
    ...
    g_Stm.stmConfig.ticks           = 100000u;  // 1us
...
void delayMs(uint32 ms)
{
    ...
    start = IfxStm_get(&MODULE_STM0);
    waitTicks = IfxStm_getTicksFromMilliseconds(&MODULE_STM0, ms);

    while ((IfxStm_get(&MODULE_STM0) - start) < waitTicks)
    {
    }
}
