...
static void Door_Motor_Drive(sint32 error, sint32 abs_err)
{
    ...
    uint32 duty;
    if      (abs_err >= DOOR_MAX_COUNT * 0.5) duty = (uint32)(0.5f * DOOR_PWM_PERIOD);
    else if (abs_err >= DOOR_MAX_COUNT * 0.25) duty = (uint32)(0.4f * DOOR_PWM_PERIOD);
...
static void Window_Motor_Drive(sint32 error, sint32 abs_err)
{
    ...
    uint32 duty;
    if      (abs_err >= WINDOW_MAX_COUNT * 0.5) duty = (uint32)(0.5f * WINDOW_PWM_PERIOD);
    else if (abs_err >= WINDOW_MAX_COUNT * 0.25) duty = (uint32)(0.4f * WINDOW_PWM_PERIOD);
